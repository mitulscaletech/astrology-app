  ...
  "inline-flex text-xs md:text-sm lg:text-small border-2 xl:text-base 2xl:text-lg gap-1 md:gap-1.5 2xl:gap-2.5 items-center justify-center tracking-[0.04em] uppercase whitespace-nowrap font-semibold ring-offset-secondary focus-visible:outline-none focus-visible:ring-0 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed relative",
  {
    variants: {
      variant: {
        ...
        "secondary-10": "bg-secondary/10 text-secondary hover:bg-secondary/20 border-transparent",
      ...
      size: {
        ...
        sm: "h-9 md:h-11 xl:h-12 px-4 md:px-5 xl:px-6",
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, rounded, asChild, isLoading, children, ...props }, ref) => {
    ...
    if (asChild) {
      ...
      return React.cloneElement(child as React.ReactElement, {
        ...
        children: (
          <>
            {isLoading && <Spinner />}
            <span className={isLoading ? "opacity-0" : "inline-flex items-center gap-1 md:gap-1.5 2xl:gap-2.5"}>
